...
#ifndef	OM_MOTOR_MASTER_H
#define OM_MOTOR_MASTER_H
...
/**
  ...
  concern yourself as to how they are achieved.  The OMMotorMaster library automatically
  ...
  a two-argument form of OMMotorMaster::move() is provided which only requires you to
  ...
  of movement is controlled by OMMotorMaster::maxStepRate() and OMMotorMaster::maxSpeed(),
  ...
  and their effects on the placement and speed of the motor over time:
   ...
   To set the type of easing applied to subsequent moves, use the OMMotorMaster::easing()
   ...
   In addition to performing complex single moves, the OMMotorMaster class also
   ...
   a natural path.  Using the OMMotorMaster::plan() and OMMotorMaster::planRun() methods, you can plan out
   ...
   For more information on how to plan interleaved moves, see OMMotorMaster::plan()
 ...
 per second, or once every 200uS.
   ...
   OMMotorMaster::move() ) and moves executed during a planned movement, you may
   use the OMMotorMaster:setMaxSpeed() command. Setting the maximum speed, instead
...
/**
  ...
  For information on complex movements and motion profiles with the OMMotorMaster
  ...
  @code
   ...
   // sketch before including the OMMotorMaster header
...
private:
	...
	static void _setTravelConst(OMMotorMaster::s_splineCal*);

	static float _qEaseCalc(OMMotorMaster::s_splineCal*, float);
	static float _qInvCalc(OMMotorMaster::s_splineCal*, float);
	...
	static float(*f_easeCal)(OMMotorMaster::s_splineCal*, float);
